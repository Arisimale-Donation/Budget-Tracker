...
class SubAdminDashboard {
    ...
    updateBalanceDisplay() {
        ...
        // If allocated is 0 (funds returned), remaining should be 0
        const remaining = allocated === 0 ? 0 : allocated - used;
        
        // If funds were returned (allocated is 0 but we have used amount),
        // use the originalAllocation for percentage calculation
        const baseAmount = allocated === 0 && used > 0 ? (this.userData.originalAllocation || used) : allocated;
        const usagePercent = baseAmount > 0 ? dbHelpers.calculatePercentage(used, baseAmount) : 0;
        ...
        // Update status indicator and alerts only if there are still allocated funds
        if (allocated > 0) {
            this.updateStatusIndicator(usagePercent);
            this.showUsageAlert(usagePercent);
        } else {
            // Hide alerts when all funds are returned
            document.getElementById('alertMessage').classList.add('hidden');
            // Reset status indicator
            const statusElement = document.getElementById('balanceStatus');
            statusElement.className = 'w-3 h-3 rounded-full bg-gray-400';
        }
    }
    ...
    async returnFunds() {
        ...
        try {
            const uid = auth.currentUser.uid;
            const currentUsed = this.userData.used || 0;
            const originalAllocation = this.userData.allocated || 0;

            // Update user's data keeping the used amount and storing original allocation
            await dbRefs.users.child(uid).update({
                allocated: 0,
                used: currentUsed,
        ...
        try {
            ...
            suggestionList.forEach(suggestion => {
                ...
                suggestionItem.innerHTML = `
                    ...
                    <button 
                        class="add-suggestion-btn bg-indigo-100 text-indigo-700 px-3 py-1 rounded-lg hover:bg-indigo-200 transition duration-200"
                ...
                const button = suggestionItem.querySelector('.add-suggestion-btn');
                button.addEventListener('click', () => {
                    this.addSuggestedItem(
                        button.dataset.name,
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Export for global use after initialization
