        ...
        try {
            ...
            Object.entries(users).forEach(([uid, user]) => {
                if (user.role === 'sub_admin') {
                    ...
                    const originalAllocation = user.originalAllocation || allocated;
                    // If allocated is 0 (funds returned), remaining should be 0
                    const remaining = allocated === 0 ? 0 : allocated - used;
                    // Calculate usage percent based on original allocation if funds were returned
                    const baseAmount = allocated === 0 && used > 0 ? originalAllocation : allocated;
